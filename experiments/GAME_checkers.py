...
class Tale:
    def __init__(self, x, y, iswhite, isplayable=True):
        ...
        self.rect = self.image.get_rect(topleft=(self.x * 80, self.y * 80))
        self.color = GRAY
        self.ishighlighted = False
        self.highlightsurface = pygame.Surface((80, 80))
    ...
    def update(self):
        ...
        screen.blit(self.highlightsurface, (self.x * 80, self.y * 80))
...
while running1:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            mousex, mousey = event.pos
            if event.button == 1:
                clicked_tale = Tale(math.floor(mousex / 80), math.floor(mousey / 80), True, True)
                for i in tales:
                    for j in i:
                        if clicked_tale == j:
                            clicked_tale = j
    ...
    for i in blacks:
        i.update()
    pygame.draw.rect(screen, BLACK, (80, 80,640, 640), 1)
    # global update
    pygame.display.flip()
    # endgame
    if len(whites) == 0:
...
if whitewins:
    print("Congratulations Player BLACK!")
else:
